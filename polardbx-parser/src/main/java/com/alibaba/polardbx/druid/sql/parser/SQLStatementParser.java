...
import com.alibaba.polardbx.druid.sql.ast.statement.SQLCreateTableStatement;
import com.alibaba.polardbx.druid.sql.ast.statement.SQLCreateTriggerStatement;
import com.alibaba.polardbx.druid.sql.ast.statement.SQLCreateTriggerStatement.TriggerType;
...
import com.alibaba.polardbx.druid.sql.ast.statement.SQLInsertInto;
...
import static com.alibaba.polardbx.druid.sql.parser.Token.MULTI_LINE_COMMENT;
import static com.alibaba.polardbx.druid.sql.parser.Token.RESTRICT;
...
public class SQLStatementParser extends SQLParser {
    ...
    public SQLStatementParser(ByteString sql) {
        this(sql, null);
    }
    ...
    public SQLStatementParser(ByteString sql, DbType dbType) {
    ...
    public void checkUnsupportedStatement(List<SQLStatement> statementList) {
        for (SQLStatement statement : statementList) {
            if (statement instanceof SQLUnsupportedStatement) {
                throw new SQLUnsupportedException((SQLUnsupportedStatement) statement);
            }
        }
    }
    ...
    public void parseStatementList(List<SQLStatement> statementList, int max, SQLObject parent) {
        ...
        for (int i = 0; ; i++) {
            ...
            if (DbType.mysql == getDbType()) {
                ...
                case KILL:
                case UPSERT:
                    // Throw when multi stmt without semi.
                    if (stmt_start) {
            ...
            case EXPLAIN: {
                if (lexer.getMark() != null && this.lexer.token == lexer.getMark().token) {
                    throw new ParserException("syntax error. " + lexer.info());
                }
...
//                        statementList.add(stmt);
            ...
            case CASE: {
                ...
                if (stmt instanceof MySqlCaseStatement) {
                    ...
                    if (((MySqlCaseStatement) stmt).getElseItem() != null) {
                        for (SQLStatement statement : ((MySqlCaseStatement) stmt).getElseItem().getStatements()) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.SLOW_SQL_CCL) && isEnabled(SQLParserFeature.DrdsCCL)) {
                SQLStatement stmt = parseSlowSqlCcl();
                statementList.add(stmt);
                stmt.setParent(parent);
                continue;
            }

            if (lexer.identifierEquals(Constants.HEATMAP)) {
                SQLStatement stmt = parsePartitionsHeatmap();
                statementList.add(stmt);
                stmt.setParent(parent);
                continue;
            }
    ...
    public SQLStatement parseDrop() {
        ...
        default:
            ...
            } else if (isEnabled(SQLParserFeature.DrdsCCL) && lexer.identifierEquals(FnvHash.Constants.CCL_RULE)) {
    ...
    protected FullTextType parseFullTextType() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.CHARFILTER)) {
            ...
            throw new ParserException(
                "type of full text must be [CHARFILTER/TOKENIZER/TOKENFILTER/ANALYZER/DICTIONARY] .");
        }
        lexer.nextToken();
        return textType;
    }
    ...
    public SQLStatement parseRefresh() {
        ...
        if (isEnabled(SQLParserFeature.DrdsMisc)) {
            if (lexer.identifierEquals("TOPOLOGY")) {
                ...
                return new DrdsRefreshLocalRulesStatement();
            }
        }
    ...
    /**
     * DROP CCL_RULE rule1 [, rule2 ...]
     */
    public SQLStatement parseDropCclRule() {
        if (lexer.token() == Token.DROP) {
            lexer.nextToken();
        }
        acceptIdentifier("CCL_RULE");
        DrdsDropCclRuleStatement stmt = new DrdsDropCclRuleStatement();
        ...
        stmt.setRuleNames(ruleNames);
        return stmt;
    }

    public SQLStatement parseDropCclTrigger() {
        if (lexer.token() == Token.DROP) {
            lexer.nextToken();
        }
        acceptIdentifier("CCL_TRIGGER");
        DrdsDropCclTriggerStatement stmt = new DrdsDropCclTriggerStatement();
        if (lexer.token == Token.IF) {
            lexer.nextToken();
            accept(Token.EXISTS);
            stmt.setIfExists(true);
        }
        List<SQLName> triggerNames = new ArrayList<SQLName>();
        SQLName sqlName = exprParser.name();
    ...
    public SQLStatement parsePurge() {
        ...
        if (lexer.identifierEquals("TRANS")) {
            lexer.nextToken();
            DrdsPurgeTransStatement stmt = new DrdsPurgeTransStatement();
            return stmt;
        }
    ...
    protected void parsePrivileages(List<SQLPrivilegeItem> privileges, SQLObject parent) {
        for (; ; ) {
            ...
            if (lexer.token == Token.ALL) {
                ...
                } else if (lexer.identifierEquals("AUDIT")) {
                    acceptIdentifier("AUDIT");
                    acceptIdentifier("LOG");
                    privilege = "SHOW AUDIT LOG";
                } else {
                    privilege = "SHOW";
                }
        ...
        if (lexer.token == Token.TABLE) {
            ...
            for (; ; ) {
                if (lexer.token == Token.DROP) {
                    ...
                    item.setNumber(num);
                    stmt.addItem(item);
                } else if ((stmt.getClusteredBy().size() > 0 || stmt.getSortedBy().size() > 0)
                    && lexer.token == INTO) {
                    lexer.nextToken();
    ...
    public SQLStatement parseCreate() {
        ...
        default:
            ...
            } else if (lexer.identifierEquals(FnvHash.Constants.JOINGROUP)) {
                lexer.reset(markBp, markChar, Token.CREATE);
    ...
    public SQLStatement parseCreateTableGroup() {
        ...
        if (lexer.identifierEquals("LOCALITY")) {
            ...
            SQLExpr locality = this.exprParser.expr();
            stmt.setLocality(locality);
        }

        return stmt;
    }

    public SQLStatement parseCreateJoinGroup() {
        accept(Token.CREATE);
        acceptIdentifier("JOINGROUP");

        SQLCreateJoinGroupStatement stmt = new SQLCreateJoinGroupStatement();
    ...
    protected SQLStatement parseDropJoinGroup() {
        if (lexer.token == Token.DROP) {
            lexer.nextToken();
        }
        ...
        SQLDropJoinGroupStatement stmt = new SQLDropJoinGroupStatement();
        stmt.setDbType(dbType);
        ...
        if (this.dbType == DbType.mysql) {
            ...
            for (; ; ) {
                if (lexer.identifierEquals(FnvHash.Constants.DISTRIBUTED)) {
                    ...
                    continue;
                } else if (lexer.identifierEquals("TABLE_PROPERTIES")) {
                    lexer.nextToken();
                    accept(Token.EQ);
    ...
    */
    ...
    public SQLStatement parseMergeTableGroup() {
        acceptIdentifier("TABLEGROUPS");
        SQLMergeTableGroupStatement stmt = new SQLMergeTableGroupStatement();
        boolean first = true;
        List<SQLName> SourceTableGroupNames = new ArrayList<SQLName>();
        do {
            if (!first) {
                accept(Token.COMMA);
            }
            SQLName tableGroupName = this.exprParser.name();
            SourceTableGroupNames.add(tableGroupName);
            first = false;
        } while (lexer.token() == Token.COMMA);
        stmt.setSourceTableGroup(SourceTableGroupNames);
        accept(INTO);
        stmt.setTargetTableGroup(this.exprParser.name());
    ...
    public SQLStatement parseMerge() {
        ...
        if (lexer.identifierEquals("TABLEGROUPS")) {
            return parseMergeTableGroup();
        }
                    ...
                    case IDENTIFIER: {
                        ...
                        if (hash == FnvHash.Constants.DATE) {
                            ...
                            if (expr instanceof SQLName) {
                                ...
                                lexer.info();
                                throw new ParserException(
                                    "insert value error, token " + lexer.stringVal() + ", line " + lexer.posLine
                                        + ", column " + lexer.posColumn, lexer.posLine, lexer.posColumn);
                            }
                            expr.setParent(values);
                        }
                        break;
...
//                        }
    ...
    protected void parseValueClause(List<SQLInsertStatement.ValuesClause> valueClauseList
        ...
        , int columnSize
        , SQLObject parent) {
        final boolean optimizedForParameterized =
            lexer.isEnabled(SQLParserFeature.OptimizedForForParameterizedSkipValue);
...
//            lexer.nextTokenValue();
    ...
    public SQLAlterSequenceStatement parseAlterSequence() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.CHANGE)) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.SIMPLE)) {
                ...
                lexer.nextToken();
            } else if (lexer.identifierEquals(FnvHash.Constants.NEW)) {
                stmt.setChangeToNew(true);
                lexer.nextToken();
            } else if (lexer.identifierEquals(FnvHash.Constants.NEW)) {
                stmt.setChangeToNew(true);
                lexer.nextToken();
    ...
    protected SQLStatement parseSlowSqlCcl() {
        DrdsSlowSqlCclStatement drdsSlowSqlCclStatement = new DrdsSlowSqlCclStatement();
        if (lexer.identifierEquals(FnvHash.Constants.SLOW_SQL_CCL)) {
            lexer.nextToken();
        }
        SQLName operation = exprParser.name();
        drdsSlowSqlCclStatement.setOperation(operation);
        List<SQLExpr> sqlExprs = new ArrayList<SQLExpr>();
        drdsSlowSqlCclStatement.setSqlExprs(sqlExprs);
        while (lexer.token != Token.EOF) {
            sqlExprs.add(exprParser.expr());
        }
        return drdsSlowSqlCclStatement;
    }

    protected SQLStatement parsePartitionsHeatmap() {
